...
#define SENSOR_DATA_N_VAR 				5
#define SENSOR_DATA_SIZE 				((uint32_t)sizeof(struct SensorData))
/* Start @ of user Flash area */
#define FLASH_USER_START_ADDR   		ADDR_FLASH_PAGE_20
/* End @ of user Flash area */
#define FLASH_USER_END_ADDR     		ADDR_FLASH_PAGE_254 + FLASH_PAGE_SIZE
#define FLASH_TOTAL_DATA_PAGES			((uint8_t) ((FLASH_USER_END_ADDR - FLASH_USER_START_ADDR) / FLASH_PAGE_SIZE))
#define FLASH_GLOBAL_DATA_IDX 			FLASH_GLOBAL_CONFIG_ADDR +  sizeof(uint32_t)
// I2C allows max 2^16 bits (2^13 bytes = 4 pages)
#define MAX_SENSOR_DATA_BYTES 			((uint16_t)(FLASH_PAGE_SIZE - (FLASH_PAGE_SIZE % SENSOR_DATA_SIZE)))
#define MAX_SENSOR_DATA_ARR_LEN 		((uint16_t)(MAX_SENSOR_DATA_BYTES/SENSOR_DATA_SIZE))
...
struct SensorData s_data_array[MAX_SENSOR_DATA_ARR_LEN];
...
void Flash_Save_Bulk_Data(struct SensorData *bulk_data, uint8_t len,
							uint8_t page, uint16_t global_nth_page);
uint32_t Flash_Write_Sensor_Data(uint32_t start_addr, uint32_t end_addr,
								struct SensorData *s);
